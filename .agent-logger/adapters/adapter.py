...
def main():
    ...
    out = {"schema_version":1, "agent":AGENT, "event":event, "session_id":d.get("session_id") or d.get("sessionId") or "unknown", "turn_id":d.get("turn_id") or d.get("turnId") or "", "prompt":d.get("prompt") or d.get("user_prompt") or "", "model":d.get("model") or "unknown", "timestamp":d.get("timestamp"), "cwd":d.get("cwd") or os.getcwd(), "status":event.removeprefix("turn.") if event.startswith("turn.") else event}
